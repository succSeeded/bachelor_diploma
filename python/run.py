...
model.find_eqpoints()
random.seed()
randpoint = np.array([0.0, random.random()*model.Ox2,
            0.0, random.random()*model.Ox4, 0.0])
model.integrate_on_plane(randpoint, T=35)
...
# randpoints = np.array([[random.random()*model.Ox1, random.random()*model.Ox2,
#             random.random()*model.Ox3, model.Ux4 + random.random()*(model.Ox4-model.Ux4), random.random()*model.Ox5] for i in range(10)])
# model.plot_transitions(randpoint)
# model.integrate_at_point(randpoint, T=30000.0, disable_plot=True)
...
# points = np.array([[x1, 0.0, 0.0, model.Ux4, 0.0] for x1 in np.linspace(1.0, 100000.0, num = 5)])
...
# model.integrate_at_points(points, T=3000, init_points=True)
# set_printoptions(suppress=True, precision=6)
# model.eqpoints = np.vstack((model.eqpoints, pot_eqpoint))
# for i in range(1,model.eqpoints.shape[0]):
